...
/**/
// the pin to connect our warning LED to
#define WARNING_LED 7

// RSSI must be stronger than this to trigger a warning
#define RSSI_THRESHOLD -60

// ignore devices older than this (in msec)
#define STALE_TIME_MSEC 5000 

/**/
// this function will be called whenever a device is added to the
// list of devices that we maintain.
// we will go through the list and if the RSSI of any device is stronger
// than a specific limit, we turn on a LED.
// we ignore any records that are too old.
void proximity_check()
{
  // assume that there are no devices with strong enough RSSI values
  bool warn = false;
  
  // get the current timestamp.
  uint32_t now = millis();
  
  // loop over all the records in our array
  for (uint8_t i=0; i<ARRAY_SIZE; i++)
  {
    // obtain local pointer to record
    node_record_t* record = &records[i];

    // if this record is too old, ignore it.
    if (now - record->timestamp > STALE_TIME_MSEC)
    {
      // move immediately on to next record
      continue;
    }

    // is the RSSI strong enough?
    if (record->rssi >= RSSI_THRESHOLD)
    {
      // it is, so remember that fact, and exit the loop
      warn = true;
      break;
    }
  }

   Serial.println(warn ? "WARN" : "OK");

  digitalWrite(WARNING_LED, warn);
}

void setup()
{
  /**/
  // code added to initialise our warning LED
 ...
 * if verbose/debug output is requested */
  ...
  /**/
  // call our proximity checking function, passing a pointer to the record we just initialised
  proximity_check();
